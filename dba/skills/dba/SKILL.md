...
The 1-hour TTL matches Anthropic's prompt-cache window — `/codehealth` followed by `/dba` 40 minutes later still hits both layers (this disk cache and the prompt cache when the next skill primes its first agent).
