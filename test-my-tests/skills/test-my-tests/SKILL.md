...
Use the agent template (`test-agent.md`). The template places shared content (codebase snapshot, languages, ground rules, output format) before the `---` divider to form a cacheable prompt prefix.
...
1. In `test-agent.md`: replace `{codebase_snapshot}` with the snapshot from Step 2.5
2. In `test-agent.md`: replace `{path}`, `{languages}`, `{focus}`, `{known_issues}`

For each reviewer:
1. Read `test-agent.md` from this skill's directory
2. Replace `{codebase_snapshot}` with the snapshot
3. Replace `{path}` with the target path
4. Replace `{languages}` with the confirmed language list
5. Read the reviewer's criteria file from `reviewers/{reviewer}.md`. If the file does not exist, skip that reviewer and warn the user. Replace `{reviewer_criteria}` with the file contents.
6. Replace `{reviewer}` with the reviewer name
7. If the user specified a focus area, replace `{focus}` with the focus block below. If no focus, replace with empty string.
8. If dcat issues were found, replace `{known_issues}` with a `## Known Issues (skip these)` section listing them. Otherwise replace with empty string.
9. For overlapping reviewers, append the relevant scope boundary rule from the Scope Boundaries section to the agent prompt.
