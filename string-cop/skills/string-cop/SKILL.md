...
The 1-hour TTL matches Anthropic's prompt-cache window.
...
- Shared prefix placeholders (`{string_inventory}`, `{path}`, `{render_layers}`, `{focus}`, `{known_issues}`) resolve to the **same value** for all agents. Resolve these once and reuse the identical string.
